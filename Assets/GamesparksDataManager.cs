...
public class GamesparksDataManager : IDataManager
{
    ...
    public void GetCallPhotoTour5()
    {
        ...
        bool callme = false;
        int loadint2 = 0;
        string dayStripe = "Fields.division.4";// + Tournament.tournament.division;
    ...
    {
        new LeaderboardDataRequest().SetLeaderboardShortCode("News1").SetEntryCount(10).Send((resp2) =>
        {
            foreach (var k in resp2.Data)
            {
    ...
    {
        new LeaderboardDataRequest().SetLeaderboardShortCode("urlsLast")
            .SetEntryCount(10).Send((response) =>
            {
                if (!response.HasErrors)
                {
                    ...
                    {
                        PortalNetwork.THIS.urlAdd(entry.JSONData["userid"] + "$" + "https://platform-lookaside.fbsbx.com/platform/profilepic/?" + entry.JSONData["url"]);
                    }
    ...
    public void SaveControl()
    {
        ...
        //if (OpenAppLevel.THIS.printScores != 0)
        //{
            new LogEventRequest_InputScore()
                .Set_TRACK(url1)
                .Set_score(OpenAppLevel.THIS.printScores)
                .Set_league("bronze")
                .Set_division("4")
        ...
        if (PlayerPrefs.GetInt("sum1") == 0)
        {
            new LogEventRequest_InputScore()
                ...
                .Set_division(Tournament.tournament.division.ToString())
                .Set_date(DateTime.Now.ToString()).Send((resp12) =>
                {
